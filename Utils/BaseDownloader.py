...
class BaseDownloader():
    ...
    def __init__(self, dl_config, ep_details, session=None):
        ...
        # ✅ Get the default subtitle language
        self.default_subtitle_lang = ep_details.get('default_subtitle_lang', None)
    ...
    def _get_raw_stream_data(self, url, stream=True, header=None):
        ...
        if self.use_http_client:
            ...
            for _ in range(max_redirects):
                ...
                if response.status in [200, 206]:  # Success
                    return response
    ...
    def _download_subtitles(self):
        for sub_name in list(self.subtitles):
            sub_data = self.subtitles[sub_name]
            # ✅ Handle both old format (string) and new format (dict)
            sub_link = sub_data['src'] if isinstance(sub_data, dict) else sub_data
    ...
    def _add_subtitles(self):
        ...
        for i, (lang, sub_data) in enumerate(self.subtitles.items(), start=1):
            ...
            # ✅ Find default subtitle - prioritize API default flag, then match with default_subtitle_lang
        ...
        # Set disposition flags - mark default subtitle
        disposition_flags = []
